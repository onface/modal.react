                ...
                <div className={`${self.props.prefixClassName}-footer`}>
                    {
                        self.props.close?
                            ...
                            (
                                <Button loading={self.state.busy.close} style={{marginRight: '.5em'}} onClick={() => {
                                    self.emitClose()
                                }} >{self.props.close}</Button>
                            )
                            :
                            self.props.close(self, components)
                        :
                        null
                    }
                    {
                        self.props.ok?
                            ...
                            (
                                <Button loading={self.state.busy.ok} type="primary" style={{marginRight: '.5em'}} onClick={() => {
                                    self.emitOk()
                                }} >{self.props.ok}</Button>
                            )
                            :
                            self.props.ok(self, components)
                        :
                        null
                    }
                </div>
